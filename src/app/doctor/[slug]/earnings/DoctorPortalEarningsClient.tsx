...
interface DoctorPortalEarningsClientProps {
  doctor: any
  helperBoys: any[]
...
export default function DoctorPortalEarningsClient({
  doctor,
  helperBoys,
  helperAttendance,
  doctorAttendance,
  ...
  branchAppointments,
  monthParam
}: DoctorPortalEarningsClientProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 p-6 bg-slate-50 dark:bg-slate-900 rounded-3xl min-h-screen text-slate-800 dark:text-slate-100">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200/60 dark:border-slate-800 pb-5">
        <div className="space-y-1">
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-teal-600 hover:text-teal-800 dark:text-teal-400 dark:hover:text-teal-350 transition cursor-pointer mb-2 font-semibold"
          >
            <ArrowLeft className="w-4 h-4" />
            Back to Dashboard
          </button>
          <h1 className="text-2xl font-sans font-bold text-transparent bg-clip-text bg-gradient-to-r from-teal-600 to-cyan-600 dark:from-teal-400 dark:to-cyan-400">
            Salary Computation breakdown
          </h1>
          <p className="text-xs text-slate-450 dark:text-slate-400 font-light uppercase tracking-wider">
        ...
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            className="px-3.5 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-teal-900/40 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 outline-none transition"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-white/75 dark:bg-slate-950/70 border border-slate-200/60 dark:border-slate-800 backdrop-blur-xl rounded-3xl p-5 flex flex-col justify-between shadow-md">
          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Your Payout</span>
          <div className="mt-2 text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-teal-600 to-cyan-500 dark:from-teal-450 dark:to-cyan-400 font-mono">
            INR {Math.round(netPayout).toLocaleString()}
          </div>
          <span className="text-[10px] text-slate-450 mt-1">Calculated net monthly salary</span>
        </div>
        ...
        <div className="bg-white/75 dark:bg-slate-950/70 border border-slate-200/60 dark:border-slate-800 backdrop-blur-xl rounded-3xl p-5 flex flex-col justify-between shadow-md">
          ...
          <span className="text-[10px] text-slate-455 mt-1">{absencesCount} absent days logged</span>
        </div>
        ...
        <div className="bg-white/75 dark:bg-slate-950/70 border border-slate-200/60 dark:border-slate-800 backdrop-blur-xl rounded-3xl p-5 flex flex-col justify-between shadow-md">
          ...
          <span className="text-[10px] text-slate-455 mt-1">Target sharing: {(doctor.profit_sharing_target || 'both').toUpperCase()}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white/75 dark:bg-slate-950/70 border border-slate-200/60 dark:border-slate-800 backdrop-blur-xl rounded-3xl p-6 space-y-4 shadow-md">
            <h3 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5 border-b border-slate-150/40 dark:border-slate-800 pb-2">
            ...
            <div className="space-y-3 text-xs">
              ...
              {doctorRule === 'present_days_only' && absencesCount > 0 && (
                <div className="space-y-2 border-t border-dashed border-slate-200/60 dark:border-slate-800 pt-3">
                  ...
                  <div className="flex justify-between border-t border-slate-100 dark:border-slate-800 pt-2 font-bold text-slate-850 dark:text-slate-100">
          ...
          {doctor.compensation_type === 'percentage' && (
            <div className="bg-white/75 dark:bg-slate-950/70 border border-slate-200/60 dark:border-slate-800 backdrop-blur-xl rounded-3xl p-6 space-y-4 shadow-md">
              ...
              <div className="space-y-3 text-xs">
                ...
                <div className="flex justify-between border-t border-slate-100 dark:border-slate-805 pt-2 font-bold text-slate-800 dark:text-slate-200">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white/75 dark:bg-slate-950/70 border border-slate-200/60 dark:border-slate-800 backdrop-blur-xl rounded-3xl p-6 space-y-4 shadow-md">
            ...
            <div className="space-y-4 font-mono text-[11px] text-slate-750 dark:text-slate-350 leading-relaxed bg-slate-50 dark:bg-slate-950/80 p-4 border border-slate-150/40 dark:border-slate-800 rounded-2xl">
              {doctor.compensation_type === 'percentage' ? (
                <>
              ...
              <div>
                <p className="text-[10px] text-slate-400 uppercase">Step E: Subtract Fines & Deductions (Fines)</p>
                <p className="font-semibold mt-0.5 text-slate-900 dark:text-slate-100">
          ...
          <div className="bg-white/75 dark:bg-slate-950/70 border border-slate-200/60 dark:border-slate-800 backdrop-blur-xl rounded-3xl p-4 flex gap-3 bg-amber-50/50 border-amber-200/60 dark:bg-amber-950/10 dark:border-amber-900/30">
